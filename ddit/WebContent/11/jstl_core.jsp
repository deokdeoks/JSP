...
// 	웹 애플리케이션의 핵심 공용 기능을 단순한 태그로 캡슐화한 웹 애플리케이션 프로그램 개방환경
//  개발환경
// 	1.http://seach.maver.org
//			jstl-1.2.jar
//			jstl-api-1.2.2.jar
...
/* 	Map<String, String> params = new HashMap<String, String>();
	...
	String het = "업무 진입과 숙련과정 : 1. 자신의 업무를 파악 및 업무 준비	, 2. 업무 숙련도 향상, 3.자신의 업무와 연계된 동료들의 업무 파악, 4. 비효율적인 업무 환경 효율하는데 기여";
...
<!-- 
	...
	forEach : begin - 반복문의 초기값 
			  ...
			  var 	- pageContext.setAttribute("i", 증가값);
			  		   ${i}
			 varStatus - 반복문의 상태정보
			 				pageContext.setAttribute("status",상태정보)
			 				${stat.first} - 최초 반복문 실행시 true를 반환
			 				${stat.last} - 최종 반복문 실행시 true를 반환
			 				${stat.count} - 반복횟수
			 	step - 증가치 
 -->
...
<h3>저장(Scope) 값 저장 : pageContext, request, session, application</h3>
<!-- 
...
<c:out value = "${jumsu }"  default="value 속성값이 zero lang. String 또는 null 일때 대체 출력값"></c:out><br/>
...
<h3>jsp:include 액션 태그 대체 컨텍스트 루트 | 패스 생략한 서블릿패스를 설정</h3>
...
${exceptionMSG}<br/>
<h3>반복문과 split 병행</h3>
<c:set var="het" value="<%= het %>">
</c:set>
<c:forTokens items="${het}" delims="," var="var">
${var }<br/>
</c:forTokens>
<%
	Map<String, String> params = new HashMap<String, String>();
