	...
	$(function(){
	    ...
	    $('form[name=freeboardReplyForm]').submit(function(){
	 		if(!$('#bo_title').val().validationTITLE()){
	 			 alertPrint("제목을 바르게 입력해주세요.");
	 			
	 		}
	 		if(!$('#bo_nickname').val().validationNICKNAME()){
	 			 alertPrint("대화명을 바르게 입력해주세요.");
	 		
	 		}
	 		if(!$('#bo_pwd').val().validationPWD()){
	 			alertPrint("패스워드를 바르게 입력해주세요..");
	 	 	}
	 		if(!$('#bo_mail').val().validationMAIL()){
	 		 alertPrint("이메일를 바르게 입력해주세요..");
	 	 	}
...
<hr />
<form name="freeboardReplyForm" role="form" action="" method="post">
